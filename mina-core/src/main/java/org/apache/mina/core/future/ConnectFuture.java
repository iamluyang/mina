 ...
 * 异步连接请求返回的IoFuture 。
 *
 * 学习笔记：连接器返回的异步结果，目的是从异步future中获取会话对象。
 *
...
public interface ConnectFuture extends IoFuture {
    ...
    /**
     * 学习笔记：如果连接器与接收器连接成功，则连接器返回一个会话对象，该会话用于与对端的会话进行读写操作。
     * 这是连接器连接服务器时连接操作返回的异步结果。
     *
     ...
     *
     * 学习笔记：当连接器到接收器的连接完成，则创建一个客户端的session对象，并设置到当前future，通知等待这个会话的线程
     *
    ...
    /**
     * 学习笔记：如果连接操作通过cancel方法取消了，则这个连接future的连接状态被设置为取消（true）。
     *
     ...
     *
     * 学习笔记：返回连接器连接对端失败的原因。如果连接操作尚未完成，或者连接已经成功，
     * 则始终返回null ，否则返回异常原因
     *
    ...
    /**
     * 设置由于连接失败而捕获的异常并通知所有等待此future的线程。
     * 该方法由 MINA 内部调用。 请不要直接调用此方法。
     *
     * 学习笔记：设置连接器连接失败的异常，并让等待连接的线程立即返回。
     *
