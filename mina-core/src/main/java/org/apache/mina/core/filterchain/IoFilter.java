...
/**
 * 拦截前往IoHandler事件的过滤器，类似于 Servlet 过滤器。过滤器可用于以下目的：
 *
 ...
 *
 *   调用 onPreAdd(IoFilterChain, String, NextFilter) 来通知过滤器将被添加到过滤器链中的事件。
 ...
 *
 *   过滤器被添加到链中后，从现在开始所有事件和I/O请求都将通过该过滤器。
 ...
 *
 *   学习笔记：如果 onPostAdd 抛出异常，则该新增的过滤器节点将从链中删除。
 *   如果该过滤器已经没有被任何父引用所持有后，该过滤器的destroy()方法也将会触发。
 ...
 * <ol>
 *   当过滤器从过滤器链中准备删除前，预删除触发的事件。即当前过滤器还没有从过滤器链的双向列表中移除。
...
public abstract class IoFilter {
    ...
    /**
     * 当任何iofilterchain不再使用此过滤器时，由ReferenceCountingFilter调用，因此您可以销毁共享资源。
     *
     * 学习笔记：请注意，如果没有使用ReferenceCountingFilter包装的过滤器，则不会在
    ...
    /**
     * 在将此筛选器添加到指定的父级之前调用。
     * 请注意，如果将此筛选器添加到多个父级（过滤器链），则可以多次调用此方法。在调用init()之前不会调用此方法。
     *
     ...
     *
     * 学习笔记：在触发onPostAdd方法时，添加的过滤器实例已经真正添加到过滤器链的双向链表中了
     *
    ...
    /**
     * 过滤 IoHandler.messageSent(IoSession,Object)} 会话发送消息产生的事件
     *
