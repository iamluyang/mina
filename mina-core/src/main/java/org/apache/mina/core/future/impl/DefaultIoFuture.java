...
import org.apache.mina.core.future.api.*;
...
/**
 * 与 IoSession 关联的 IoFuture 的默认实现。
 *
...
public class DefaultIoFuture implements IoFuture {
     ...
     * 对象的等待队列，为了避免长时间的等待，因此每次等待的时间为一个DEAD_LOCK_CHECK_INTERVAL
     * 的间隔时间（相当于每隔一会线程自己醒过来，重新检查一下ready标记），由于这是一个没有超时
     * 的await，因此重复wait和自动醒来的过程，直到ready为true，才退出。
     *
    ...
    /**
     * 学习笔记：支持打断的await，表示当前等待结果的线程被打断后会抛出打断的异常
     *
     * {@inheritDoc}
     */
    @Override
    ...
    /**
     * 学习笔记：等待异步结果的阻塞方法（永不超时），当等待结果的线程被打断，异常会被捕获，并静默处理，然后静默的退出该阻塞方法
     *
     * {@inheritDoc}
     */
    @Override
    ...
    private boolean await0(long timeoutMillis, boolean interruptable) throws InterruptedException {
        // 学习笔记：endTime由当前时间和超时时间相加而成，即最后通牒时间
        long endTime = System.currentTimeMillis() + timeoutMillis;

        // 学习笔记：如果endTime时间加起来小于零
        // 如果超时时间非常大，导致当前时间毫秒数+超时时间的计算结果越界了变成负数，则设置一个MAX值长时间等待
        if (endTime < 0) {
        ...
        synchronized (lock) {
            ...
            try {
                for (;;) {
                    try {
                        // 学习笔记：在超时时间和间隔时间之间取一个较小的值，避免超时时间太长，可能不需要
                        // 等待那么长的当时间，异步结果就完成了，因此可以每隔一段时间就立即检测一下
                        long timeOut = Math.min(timeoutMillis, DEAD_LOCK_CHECK_INTERVAL);

                        // 学习笔记：等待请求的一段时间，但是每隔DEAD_LOCK_CHECK_INTERVAL秒，我们将检查是否阻塞。
                    ...
                    // 学习笔记：如果等待异步结果的线程从等待中醒来，则判断一下ready准备好了没有，或者截止时间已经到了
                    // 则从当前await阻塞方法中退出，并返回ready的状态，可能为true，或false
                    if (ready || (endTime < System.currentTimeMillis())) {
                        return ready;
                    } else {
                        // 学习笔记：如果即没有ready准备好，也没有超过超时的时间，则检测一下是否发生了死锁
     ...
     * 如果结果已经设置过则不重复设置，直接返回false，表示设置失败，否则设置回调结果和标记，
    ...
    public IoFuture addListener(IoFutureListener<?> listener) {
        ...
        // 每次只能有一个线程添加监听器，一般来说不会有多个线程同时添加监听器
        synchronized (lock) {
            // 在future结束后添加的监听器，直接触发这个监听器即可，而不需要添加到future内部到监听器容器中
            if (ready) {
    ...
    public IoFuture removeListener(IoFutureListener<?> listener) {
        ...
        // 每次只能有一个线程移除监听器，一般来说只会有一个线程
        synchronized (lock) {
            // 如果监听器还未就绪则可以删除监听器，即异步请求还未结束前可以撤销监听器，
            // 如果异步已经处理完成，则不能再撤销监听器了
            if (!ready) {
                if (listener == firstListener) {
                    // 如果第一个监听器就是要删除的对象，则看看other中有没有对象，弹出一个作为新的第一个监听器
                    ...
                    // 如果第一个监听器不是要删除的对象，则从other容器中删除对象
