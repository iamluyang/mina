...
/**
 * 表示在 IoSession 上完成异步 I/O 操作。可以使用 IoFutureListener 监听异步请求是否完成。
 *
...
public interface IoFuture {
    ...
    /**
     * 返回异步结果是否完成，完成可以是请求失败，或者请求成功。
     * 因此请求无论是成功还是失败，请求确实是结束了
     *
    ...
    /**
     * 等待异步操作，并静默线程打断异常。操作完成时将通知附加的侦听器。
     *
     * 学习笔记：等待异步结果，在等待的过程中如果线程被打断，也不会将线程打断的异常向外抛出。
     *
    ...
    /**
     * 添加一个事件listener，当这个Future完成时会收到通知。如果完成后添加监听器，则直接通知监听器。
     *
     * 学习笔记：给异步future添加的一个监听器，当异步请求结束后这个监听器将被触发，如果在添加监听器
     * 时异步结果就已经完成，则立即触发这个监听器。
     *
    ...
    /**
     * 删除现有事件listener以便在future完成时不会收到通知。
     *
     * 学习笔记：从当前异步future中移除一个监听器，如果能在异步结果完成前抓紧删除掉，则
     * 可以避免这个监听器被触发。
     *
